...
function Posts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        // 🔥 FETCH ALL POSTS (no userId filter)
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(
        `/api/post/delete-post/${postTodelete}/${currentUser._id}`,
          ...
          <div className="bg-white text-black p-6 rounded-lg w-80 text-center">
            ...
            <div className="flex justify-center gap-4 font-semibold">
              <button
                ...
                className="px-4 py-2 bg-red-600 text-white rounded font-semibold hover:bg-red-800">
                Yes
              </button>
              <button
